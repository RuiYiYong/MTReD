...
class SparseGA():
    ...
    def get_dense_pts3d(self, clean_depth=True, subsample=8):
        ...
        with open('saved_pixels.pkl', 'wb') as f:
...
def sparse_scene_optimizer(imgs, subsample, imsizes, pps, base_focals, core_depth, anchors, corres, corres2d,
    ...
    def loss_2d(K, w2cam, pts3d, pix_loss):
        ...
        with open("/media/ruiy/C07248AA7248A6CE/Users/RuiY/Documents/mast3r/Output.txt", "w") as text_file:
            text_file.write(str((loss / npix).item()) if npix != 0 else 0)
        # print(f"2D LOSS: {loss / npix if npix != 0 else 0}")
    ...
    def optimize_loop(loss_func, lr_base, niter, pix_loss, lr_end=0):
        ...
        with tqdm(total=niter) as bar:
            for iter in range(niter or 1):
                ...
                # print(f"LOSS FUNC: {loss_func(K, w2cam, pts3d, pix_loss)}\n DUST3r LOSS: {loss_dust3r(cam2w, pts3d, lossd)}")
                loss.backward()
                optimizer.step()
